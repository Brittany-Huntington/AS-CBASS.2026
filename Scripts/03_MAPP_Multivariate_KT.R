...
pp_formatted <- pp %>%
  mutate(
    sample_file = X,
    site   = str_extract(sample_file, "^[^_]+"),
    genotype    = str_extract(sample_file, "(?<=_)[A-Za-z0-9-]+")
  )

meta_cols <- c("sample_file", "site", "genotype")
...
#  FIX FOR Missing Values: Replace any residual NAs/NaNs in cor_matrix with 0
cor_matrix[is.na(cor_matrix)] <- 0
cor_matrix[is.nan(cor_matrix)] <- 0

#  Run findCorrelation safely
high_cor_idx <- findCorrelation(cor_matrix, cutoff = 0.99) #if metrics are 0.99 identical, drop (goign to keep more!)
med_cor_idx  <- findCorrelation(cor_matrix, cutoff = 0.90) #if metrics are 0.9 identical, dop (goign to keep less!)

# Get column names to remove
cols_to_remove_99 <- c(
  colnames(trait_data)[!non_zero_var_mask], # dropped zero-var traits
  colnames(trait_data_clean)[high_cor_idx]  # dropped > 0.99 traits
)

cols_to_remove_90 <- c(
  colnames(trait_data)[!non_zero_var_mask], # dropped zero-var traits
  colnames(trait_data_clean)[med_cor_idx]   # dropped > 0.90 traits
)

# Subset your formatted dataframes cleanly
...
# =========================================================================
# 3. Hierarchical Bootstrapped Clustering (pvclust)
# =========================================================================
prep_for_pvclust <- function(df) {
  traits_only <- df[, !(names(df) %in% c("sample_file", "site", "genotype", "GroupingProperty"))]
...
cat("Running bootstraps for pp_99...\n")
fit_99 <- pvclust(pv_data_99, method.hclust = "average", method.dist = "correlation", 
                  nboot = 500, parallel = TRUE, iseed = 123)

cat("Running bootstraps for pp_90...\n")
fit_90 <- pvclust(pv_data_90, method.hclust = "average", method.dist = "correlation", 
                  nboot = 500, parallel = TRUE, iseed = 123)

# Multiverse ANOVA across K = 4:8
...
meta_99 <- pp_99_species %>% dplyr::select(sample_file, site, genotype, GroupingProperty, Species)
meta_90 <- pp_90_species %>% dplyr::select(sample_file, site, genotype, GroupingProperty, Species)
...
# Pairwise PERMANOVA (Fixed data argument for pp_90)
...
# =========================================================================
# Homogeneity of Dispersion Tests (betadisper)
# =========================================================================

# 1. Dispersion for pp_99
...
# =========================================================================
# 1. Run nMDS on Scaled Matrix
# =========================================================================
# autotransform = FALSE prevents metaMDS from log-transforming negative scaled values
...
# =========================================================================
# 2. Fit Photophysiological Trait Vectors (envfit)
# =========================================================================
# envfit calculates direction & correlation (R2) for every metric in the matrix
ef <- envfit(nmds_fit, matrix_99_scaled, permutations = 500)

# Extract vector coordinates and p-values into a clean dataframe
...
# Filter to the TOP 10 strongest metric drivers (high R2, significant p < 0.001)
top_vectors <- vector_scores %>%
  filter(p_val < 0.002) %>%
  arrange(desc(r2)) %>%
  head(11)

cat("--- TOP 10 METRIC DRIVERS OF SPECIES ORDINATION ---\n")
print(top_vectors)

# Extract sample NMDS coordinates
nmds_scores <- as.data.frame(scores(nmds_fit, display = "sites")) %>%
  bind_cols(meta_99 %>% dplyr::select(Species, genotype))

# =========================================================================
# 1. Clean Vector Labels for Publication
# =========================================================================
top_vectors_clean <- top_vectors %>%
  mutate(
    # Extract functional trait family + lighting phase for clean labeling
    ...
    # Create clean short label (e.g., "mQuant (L3Camp)", "rABQ (Mean)")
...
# =========================================================================
# 2. Plot nMDS with Cleaned Vector Overlay
# =========================================================================
# Set arrow length multiplier so vectors fit nicely inside plot bounds
arrow_mult <- 50.2 
